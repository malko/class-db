...
* @since 2004-12-02 use the show fields query in place of a select statement and add extended_info mode to the get_fields method
* @since 2004-12-03 now the associative_array_from_q2a_res method won't automaticly ksort the results 
*/
  ...
  */
  function dump_to_file($out_file,$droptables=TRUE,$gziped=TRUE){
    ...
    foreach($tables as $table){
      ...
      if($tabledatas = $this->select_to_array($table)){ # si on a des données ds la table on les mets
        if($gziped)
          gzwrite($fout,"\n# `$table` DATAS\n\n");
        else
          fwrite($fout,"\n# `$table` DATAS\n\n");
